  ...
  return (
    <div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-3">
      {(pricingPlan as PricingPlan[]).map((plan, idx) => (
        <RevealAnimation key={idx + 1} delay={0.6 + idx * 0.1}>
          <div
            className={cn(
            ...
            <div className={cn(plan.bgImage && 'rounded-[12px] bg-white p-5 sm:p-8 dark:bg-black')}>
              ...
              <ul className="relative list-none space-y-2.5">
                {plan.features.map((feature, i) => (
                  <li key={i + 1} className="flex items-center gap-2.5">
                    {/* Icon */}
                    <span
                      className={cn(
                        'flex size-5 shrink-0 items-center justify-center rounded-full',
                        feature.enabled ? 'bg-secondary dark:bg-accent/80' : 'dark:bg-background-9 bg-white',
                        !feature.enabled && plan.planType === 'featured' && 'bg-background-4 dark:bg-background-9',
                      )}>
                      <CheckIcon
                        className={cn(
                          feature.enabled
                            ? 'dark:fill-background-8 fill-white'
                    ...
                    <span
                      className={cn(
                        ...
                        feature.enabled
                          ? 'text-secondary dark:text-accent/80'
                          : 'text-secondary/60 dark:text-accent/40',
                      )}>
                      {feature.label}
                    </span>
                  </li>

      ...
      {(pricingPlan as PricingPlan[]).map((plan, idx) => {
        ...
        return (
          <RevealAnimation key={idx} delay={0.6 + idx * 0.1}>
            <div
              className={cn(
                'col-span-12 mx-auto w-full max-w-[408px] rounded-[20px] lg:col-span-4',
                plan.bgImage ? 'bg-cover bg-center bg-no-repeat p-2.5' : 'bg-background-3 dark:bg-background-7 p-8',
              )}
              style={plan.bgImage ? { backgroundImage: `url(/images/ns-img-25.png)` } : {}}>
              <div className={cn(plan.bgImage && 'dark:bg-background-8 rounded-[12px] bg-white p-8')}>
                {/* Title + Desc */}
                <h3 className="text-heading-5 mb-2 font-normal">{plan.title || 'Plan Title'}</h3>
                        ...
                        <li key={i} className="flex items-center gap-2.5">
                          <span
                            className={cn(
                              ...
                              feature.enabled ? 'bg-secondary dark:bg-accent' : 'dark:bg-background-9 bg-white',
                              plan.planType === 'featured' &&
                                !feature.enabled &&

        ...
        <Accordion
          ...
          {faqData.map((faq) => (
            <AccordionItem key={faq.id} className="border-stroke-1 rounded-[20px] border" value={faq.id}>
              <AccordionTrigger
                iconClassName="size-7 data-[state=open]:bg-secondary data-[state=open]:text-white data-[state=closed]:bg-ns-green rounded-full flex items-center justify-center  transition-colors duration-300 ease-in-out"

...
interface TrustedByUsersProps {
  className?: string;
  title: string;
  description: string;
}

const TrustedByUsers: FC<TrustedByUsersProps> = ({ className, title, description }) => {
  return (
    <section className={cn(className)} aria-label="Trusted by users section">
        ...
        <div className="max-h-auto bg-secondary dark:bg-background-6 flex flex-col items-center gap-x-8 gap-y-10 rounded-[20px] py-14 lg:max-h-[280px] lg:flex-row lg:items-start">
          <div className="space-y-3 max-sm:px-3 max-sm:text-center lg:w-[37%] lg:pl-12">
            <RevealAnimation delay={0.1}>
              <h2 className="lg:text-heading-3 text-white">{title}</h2>
            </RevealAnimation>
            <RevealAnimation delay={0.2}>
              <p className="text-accent/60 px-4 md:px-0">{description}</p>
            </RevealAnimation>
          </div>
